...
pub fn end_alias(original: &str, typed: &str) -> Option<(String, String, bool)> {
    let original_cropped: Vec<(usize, &str)> = original
        ...
        .skip(
            ...
            typed.graphemes(true).count().saturating_sub(ALIAS_MAX_SIZE),
        )
        .take(ALIAS_MAX_SIZE)
        .collect();
...
pub fn validate_with_replacements(
    ...
    let last_typed_grapheme_offset = typed.graphemes(true).count().saturating_sub(1);
...
pub fn pop_grapheme_in_place(s: &mut String, graphemes: usize) {
    for _ in 0..graphemes {
        let last_chars = s.graphemes(true).next_back().unwrap_or("").chars().count();
