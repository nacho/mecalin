...
impl imp::SpeedTestTextView {
    ...
    pub(super) fn update_colors(&self, comparison: &[(GraphemeState, usize, usize, usize)]) {
        let original = self.original_text.borrow();
        let typed = self.typed_text.borrow();
        ...
        let mut color_start_iter = typed_iter;
        ...
        // the currently active one (just enough to account for the scrolling animation in all cases)
        let mut color_end_iter = typed_iter;
        ...
        for n in typed_iter.offset()..color_end_offset {
            let start_iter = buf.iter_at_offset(n);
            let end_iter = buf.iter_at_offset(n + 1);
