...
mod imp {
    ...
    #[template(resource = "/io/github/nacho/mecalin/ui/speed_test_text_view.ui")]
    #[properties(wrapper_type = super::SpeedTestTextView)]
    pub struct SpeedTestTextView {
        #[template_child]
    ...
    impl ObjectSubclass for SpeedTestTextView {
        const NAME: &'static str = "SpeedTestTextView";
        type Type = super::SpeedTestTextView;
        type ParentType = gtk::Widget;

        fn class_init(klass: &mut Self::Class) {
            klass.set_css_name("SpeedTestTextView");
    ...
    impl ObjectImpl for SpeedTestTextView {
    ...
    impl WidgetImpl for SpeedTestTextView {
    ...
    impl SpeedTestTextView {
        ...
        pub fn set_original_text(&self, text: &str) {
            *self.original_text.borrow_mut() = text.to_string();
            self.text_view.buffer().set_text(&insert_replacements(text));
            self.compare_and_update_colors();
            self.update_caret_position(true);
            self.update_scroll_position(true);
...
impl SpeedTestTextView {
    ...
    pub fn keystrokes(&self) -> Ref<'_, Vec<(Instant, bool)>> {
